...
template <>
__global__ void modulated_deformable_im2col_gpu_kernel<float>(int n,
  ...
  CUDA_KERNEL_LOOP(index, n)
  {
    ...
    for (int i = 0; i < kernel_h; ++i)
    {
      for (int j = 0; j < kernel_w; ++j)
      {
        ...
        float offset_h = __half2float(data_offset_ptr[data_offset_h_ptr]);
        float offset_w = __half2float(data_offset_ptr[data_offset_w_ptr]);
        float mask = __half2float(data_mask_ptr[data_mask_hw_ptr]);
        float val = static_cast<float>(0);
        float h_im = h_in + i * dilation_h + offset_h;
        float w_im = w_in + j * dilation_w + offset_w;
        if (h_im > -1 && w_im > -1 && h_im < height && w_im < width)
        {
          val = dmcn_im2col_bilinear<float>(data_im_ptr, width, height, width, h_im, w_im);
        }
...
template <>
__global__ void modulated_deformable_im2col_gpu_kernel<__half>(int n,
                                                       __half *data_im, __half *data_offset,  __half *data_mask,
                                                       int height,     int width,           int kernel_h, int kernel_w,
                                                       int pad_h,      int pad_w,
                                                       int stride_h,   int stride_w,
                                                       int dilation_h, int dilation_w,
  ...
  CUDA_KERNEL_LOOP(index, n)
  {
    ...
    for (int i = 0; i < kernel_h; ++i)
    {
      for (int j = 0; j < kernel_w; ++j)
      {
        ...
        if (h_im > -1 && w_im > -1 && h_im < height && w_im < width)
        {
          val = dmcn_im2col_bilinear<__half>(data_im_ptr, width, height, width, h_im, w_im);
        }
        *data_col_ptr = __float2half(val * mask);
        data_col_ptr += height_col * width_col;
      }
    }
  }
...
int DCNv2Plugin::do_initialize(){
    ...
    if (batch > 10)//默认maxbatch为32，但是网络消耗内存过多，一般内存batch不超过8
    {
        batch = 8;
    }
